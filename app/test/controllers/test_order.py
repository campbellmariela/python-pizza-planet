...
def test_get_all(app, ingredients, sizes, beverages, client_data):
    ...
    created_orders = []
    for _ in range(5):
        order = __order(shuffle_list(created_ingredients)[:3], get_random_choice(
            created_sizes), shuffle_list(created_beverages)[:3], client_data)
        created_order, _ = OrderController.create(order)
